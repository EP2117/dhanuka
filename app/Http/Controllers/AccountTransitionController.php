...
class AccountTransitionController extends Controller
{
    ...
    public function getAllCashbook(Request $request) {
        ...
        return response(compact('total','cashbook'));
    }
    ...
    public function getProfitAndLossReport(Request $request){
        $month = Carbon::now()->year($request->year)->month($request->month);
        // dd($targetDate);
        // dd($request->all());
        ...
        foreach($ah_expense as $key=>$ae){
           ...
           // $request->year=$request->year==null ? Carbon::now()->year :  $request->year;
            ...
            /*$ple->whereHas('sub_account.account_head',function($q)use($ae){
                $q->whereId($ae->id);
            })->selectRaw('*, (CASE WHEN journal_entry_id IS NULL THEN sum(IFNULL(debit,0))  ELSE sum(IFNULL(debit,0) - IFNULL(credit,0)) END)  as amount, sum(IFNULL(credit,0)) as cr_amount')->groupBy('sub_account_id');*/

            $ple->whereHas('sub_account.account_head',function($q)use($ae){
                $q->whereId($ae->id);
            })->selectRaw('*, sum(IFNULL(debit,0)) as amount, sum(IFNULL(credit,0)) as cr_amount')->groupBy('sub_account_id');
                    ...
                    foreach($ple as $k=>$p){
                        ...
                        if($p->sub_account->account_head->name==$ae->name){
                            ...
                            /**if($p->sub_account_id == 80 && !empty($p->sale_id)){
                             ...
                             /* $expense[$key][$k]->amount=$p->amount - $p->cr_amount;
                              $total_expense+=$p->amount - $p->cr_amount;
                              $total+=$p->amount - $p->cr_amount;*/
                              $expense[$key][$k]->amount=$p->amount;
                              $total_expense+=$p->amount;
                              $total+=$p->amount;
      ...
      $c_loss=$c_loss->first();
        ...
        //echo 'gp='.$gross_profit.', income='.$total_income.', expense='.$total_expense;
        //dd($net_profit);
        //ep
    ...
    public function getValuation($year){
        ...
        $where .= " AND YEAR(product_transitions.transition_date) <= " . $year;
        $p_where .= " AND YEAR(purchase_invoices.invoice_date) <= " . $year;
        $s_where .= " AND YEAR(sales.invoice_date) <= " . $year;
        

        $products = DB::table("products")
            ->where('products.is_active', 1)
            ->select(DB::raw("products.id as product_id,pt.transition_date,ps.s_valuation_amount,pt.cost_price,pt.in_cost_price,products.purchase_price,ps.s_qty,pp.p_valuation_amount,pt.entry_qty,products.product_name,products.minimum_qty, products.brand_id,pt.warehouse_id, products.product_code,uom_id,pt.in_qty,pt.out_qty,IFNULL(pt.entry_cost_price, 0) as entry_cost_price"))
            ->leftjoin(DB::raw("(SELECT product_id,product_quantity,transition_type,transition_purchase_id, warehouse_id, transition_date, branch_id,
                         ...
                         SUM(CASE  WHEN transition_type = 'in'  AND transition_entry_id IS NOT NULL THEN product_quantity * transition_purchase_price  ELSE 0 END) as entry_cost_price,
                         ...
                         SUM(CASE  WHEN transition_type = 'in'  AND (transition_adjustment_id IS NOT NULL OR transition_transfer_id IS NOT NULL OR transition_sale_id IS NOT NULL OR transition_credit_note_id IS NOT NULL) THEN cost_price ELSE 0 END ) as in_cost_price,
                         SUM(CASE  WHEN transition_type = 'out'  THEN product_quantity  ELSE 0 END) as out_qty
                         FROM product_transitions Where " . $where . " GROUP BY product_transitions.product_id)as pt"), function ($join) {
        ...
        $s_where = "1=1";
        /*if ($request->date != '') {
            $where .= " product_transitions.transition_date <= '" . $year . "'";
            $p_where .= " purchase_invoices.invoice_date <= '" . $year . "'";
            $s_where .= " sales.invoice_date <= '" . $year . "'";
        } else {
        ...
        if ($request->warehouse_id != '') {
            $where .= " AND product_transitions.warehouse_id = " . $request->warehouse_id;
            $p_where .= " AND purchase_invoices.warehouse_id = " . $request->warehouse_id;
            $s_where .= " AND sales.warehouse_id = " . $request->warehouse_id;
        }*/
        ...
        $products = DB::table("products")
            ->where('products.is_active', 1)
            ->select(DB::raw("products.id as product_id,pt.transition_date,pt.cost_price,pt.in_cost_price,products.purchase_price,pt.entry_qty,products.product_name,products.minimum_qty, products.brand_id,pt.warehouse_id, products.product_code,uom_id,uoms.uom_name,brands.brand_name,categories.category_name, pt.in_qty,pt.out_qty,IFNULL(pt.adj_in_cost_price, 0) as adj_in_cost_price,IFNULL(pt.adj_out_cost_price, 0) as adj_out_cost_price,IFNULL(pt.entry_cost_price, 0) as entry_cost_price,products.cost_price as product_cost_price"))
            ->leftjoin(DB::raw("(SELECT product_id,product_quantity,transition_type,transition_purchase_id, warehouse_id, transition_date, branch_id,
                         SUM(CASE  WHEN transition_type = 'in'  AND transition_entry_id IS NOT NULL THEN product_quantity  ELSE 0 END) as entry_qty,
                         SUM(CASE  WHEN transition_type = 'in' AND transition_entry_id IS NULL THEN product_quantity  ELSE 0 END) as in_qty,
                         SUM(CASE  WHEN transition_type = 'in'  AND transition_entry_id IS NOT NULL THEN cost_price ELSE 0 END) as entry_cost_price,
        ...
        foreach($data as $k=>$v) {
            $s_where .= " AND product_sale.product_id =".$v->product_id;
            $ps = DB::select(DB::raw("(SELECT product_sale.product_id,SUM(product_sale.total_amount) as s_valuation_amount,product_quantity as s_qty
                        FROM sales LEFT JOIN product_sale ON product_sale.sale_id = sales.id WHERE " . $s_where . "
                       GROUP BY product_sale.product_id
                       )"));
           if(count($ps) > 0) {
                $data[$k]->s_valuation_amount = $ps[0]->s_valuation_amount;
                $data[$k]->s_qty = $ps[0]->s_qty;
            } else {
                $data[$k]->s_valuation_amount =0;
                $data[$k]->s_qty = 0;
            }

            $p_where .= " AND product_purchase.product_id =".$v->product_id;
            $pp = DB::select(DB::raw("(SELECT product_purchase.product_id,SUM(product_purchase.total_amount) as p_valuation_amount
                    FROM purchase_invoices LEFT JOIN product_purchase ON product_purchase.purchase_id = purchase_invoices.id WHERE " . $p_where . "
                   GROUP BY product_purchase.product_id
                   )"));

           if(count($pp) > 0) {
                $data[$k]->p_valuation_amount = $pp[0]->p_valuation_amount;
            } else {
                $data[$k]->p_valuation_amount =0;
            }
        }
        return $data;
    }

    public function getValuation_dhanuka($year){
        $where = "1=1";
        $p_where = "1=1";
        $s_where = "1=1";
    ...
    public function getBalanceSheetReport(Request $request){
        ...
        foreach ($data as $p) {
          ...
          $bal = ((int)$p->entry_qty + (int)$p->in_qty) - (int)$p->out_qty;
            ...
            $p->s_qty = $p->s_qty == null ? 0 : (int)$p->s_qty;
             ...
             //$total_valuation += ((int)$p->entry_cost_price) + (((int)$p->p_valuation_amount + $p->in_cost_price) - (int)$p->cost_price);
            //check only for lucky asia bcoz of cost price error records
            if($p->balance == 0) {
                $p->t_valuation_amount =0;
                //$total_valuation = 0;
            } else {
                 $p->t_valuation_amount = ((int)$p->entry_cost_price) + (int)(((int)$p->p_valuation_amount + $p->in_cost_price) - (int)$p->cost_price);
                //$p->t_valuation_amount = $p->balance * round($p->product_cost_price); 
                $total_valuation += $p->t_valuation_amount;   

            }
            /*echo 'entry qty * purchase_price='.$p->entry_qty * $p->purchase_price.'<br />';
            echo 'p_valuation_amount='.$p->p_valuation_amount.'<br />';
            echo 'in cost price='.$p->in_cost_price.'<br />';
            echo 'cost price='.$p->cost_price.'<br />';
            echo 'opening_valuation_amount='.$p->opening_valuation_amount;
            dd($total_valuation);*/

            //added after dhanuka update 
        ...
        foreach($asset_account_head as $key=>$aah) {
          ...
          if($aah->name == 'Current Asset') {
            ...
            $c_asset = DB::SELECT(DB::raw("Select t.*, COALESCE(op.opening_amount,0) as op_amount, op.opening_amount_type FROM (Select sub_accounts.id as subAccId, sub_accounts.account_group_id,sub_accounts.sub_account_name, tt.* FROM sub_accounts LEFT JOIN (Select account_transitions.cash_bank_sub_account_id, (SUM(COALESCE(debit,0)) - SUM(COALESCE(credit,0))) as amount, (CASE  WHEN SUM(ABS(COALESCE(debit,0))) > SUM(ABS(COALESCE(credit,0))) THEN 'debit' ELSE 'credit' END)  as amount_type FROM sub_accounts LEFT JOIN account_transitions ON sub_accounts.id = account_transitions.cash_bank_sub_account_id WHERE YEAR(transition_date) = ".$year. " AND cash_bank_sub_account_id IN (".$ca_str.") AND is_cashbook=0 GROUP BY sub_accounts.id) as tt ON tt.cash_bank_sub_account_id = sub_accounts.id) as t LEFT JOIN (Select account_transitions.cash_bank_sub_account_id, (SUM(COALESCE(debit,0)) - SUM(COALESCE(credit,0))) as opening_amount, (CASE  WHEN SUM(ABS(COALESCE(debit,0))) > SUM(ABS(COALESCE(credit,0))) THEN 'debit' ELSE 'credit' END)  as opening_amount_type From account_transitions LEFT JOIN sub_accounts ON sub_accounts.id = account_transitions.cash_bank_sub_account_id WHERE YEAR(transition_date) < ".$year." AND  cash_bank_sub_account_id IN (".$ca_str.") AND is_cashbook=0 GROUP BY cash_bank_sub_account_id) as op ON t.subAccId=op.cash_bank_sub_account_id WHERE (t.amount IS NOT NULL OR op.opening_amount IS NOT NULL) AND t.account_group_id IS NOT NULL"));
          }
          //end current asset
          ...
          if($aah->name == 'Non-Current Asset') {
            ...
            $nc_asset = DB::SELECT(DB::raw("Select t.*, COALESCE(op.opening_amount,0) as op_amount, op.opening_amount_type FROM (Select sub_accounts.id as subAccId, sub_accounts.account_group_id, sub_accounts.sub_account_name, tt.* FROM sub_accounts LEFT JOIN (Select account_transitions.sub_account_id, (ABS(SUM(COALESCE(debit,0)) - SUM(COALESCE(credit,0)))) as amount, (CASE  WHEN SUM(ABS(COALESCE(debit,0))) > SUM(ABS(COALESCE(credit,0))) THEN 'debit' ELSE 'credit' END)  as amount_type FROM sub_accounts LEFT JOIN account_transitions ON sub_accounts.id = account_transitions.sub_account_id WHERE YEAR(transition_date) = ".$year. " AND sub_account_id IN (".$nca_str.") AND is_cashbook=0 GROUP BY sub_accounts.id) as tt ON tt.sub_account_id = sub_accounts.id) as t LEFT JOIN (Select account_transitions.sub_account_id, (ABS(SUM(COALESCE(debit,0)) - SUM(COALESCE(credit,0)))) as opening_amount, (CASE  WHEN SUM(ABS(COALESCE(debit,0))) > SUM(ABS(COALESCE(credit,0))) THEN 'debit' ELSE 'credit' END)  as opening_amount_type From account_transitions LEFT JOIN sub_accounts ON sub_accounts.id = account_transitions.sub_account_id WHERE YEAR(transition_date) < ".$year." AND  sub_account_id IN (".$nca_str.") AND is_cashbook=0 GROUP BY sub_account_id) as op ON t.subAccId=op.sub_account_id WHERE (t.amount IS NOT NULL OR op.opening_amount IS NOT NULL) AND t.sub_account_name != 'Prepaid Expense'"));
          }
        ...
        $gain_loss_profit = ($net_profit + abs($c_gain))  - abs($c_loss);

        /*$pl = $this->getProfitAndLossReport($request);
        $pl = $pl->getData(true); // convert JSON response to array
        $c_gain = empty($pl['c_gain']) ? 0 : $pl['c_gain']->amount;
        $c_loss = empty($pl['c_loss']) ? 0 : $pl['c_loss']->amount;
        $net_profit= empty($pl['net_profit']) ? 0 : $pl['net_profit'];
        $gain_loss_profit = ($net_profit + abs($c_gain))  - abs($c_loss);*/
        ...
        if(!empty($c_asset)) {
          ...
          foreach($c_asset as $casset) {  
            if(!empty($casset->opening_amount_type) && !empty($casset->op_amount)) {
        ...
        if(!empty($total_valuation)) {
          $total_c+=round($total_valuation);
        }
        if(!empty($os_amount)) {
          $total_c+=$os_amount;
        }
        ...
        //Equity & Liablity
        $total_eq_li = 0;
        $el_account_head=AccountHead::with('sub_accounts')->where('name','Equity')->orWhere('name','Current Liability')->orWhere('name','Long Term Liability')->orWhere('name','Non Current Liability')->get();
        ...
        foreach($el_account_head as $key=>$el) {
          ...
          if($el->name == 'Equity') {
            ...
            if(!empty($eq_str)) {
            $eq_query = DB::SELECT(DB::raw("Select t.*, COALESCE(op.opening_amount,0) as op_amount, op.opening_amount_type FROM (Select sub_accounts.id as subAccId, sub_accounts.account_group_id, sub_accounts.sub_account_name, tt.* FROM sub_accounts LEFT JOIN (Select account_transitions.sub_account_id, (ABS(SUM(COALESCE(debit,0)) - SUM(COALESCE(credit,0)))) as amount, (CASE  WHEN SUM(ABS(COALESCE(debit,0))) > SUM(ABS(COALESCE(credit,0))) THEN 'debit' ELSE 'credit' END)  as amount_type FROM sub_accounts LEFT JOIN account_transitions ON sub_accounts.id = account_transitions.sub_account_id WHERE YEAR(transition_date) = ".$year. " AND sub_account_id IN (".$eq_str.") AND is_cashbook=0 GROUP BY sub_accounts.id) as tt ON tt.sub_account_id = sub_accounts.id) as t LEFT JOIN (Select account_transitions.sub_account_id, (ABS(SUM(COALESCE(debit,0)) - SUM(COALESCE(credit,0)))) as opening_amount, (CASE  WHEN SUM(ABS(COALESCE(debit,0))) > SUM(ABS(COALESCE(credit,0))) THEN 'debit' ELSE 'credit' END)  as opening_amount_type From account_transitions LEFT JOIN sub_accounts ON sub_accounts.id = account_transitions.sub_account_id WHERE YEAR(transition_date) < ".$year." AND  sub_account_id IN (".$eq_str.") AND is_cashbook=0 GROUP BY sub_account_id) as op ON t.subAccId=op.sub_account_id WHERE t.amount IS NOT NULL OR op.opening_amount IS NOT NULL"));
        ...
        //start non current liability
        $adj_amount = 0;
        if($el->name == 'Non Current Liability') {
            
            $ncl_arr = array();
            $ncl_str = '';
            foreach($el->sub_accounts as $ncl) {
                array_push($ncl_arr, $ncl->id);
                if($ncl_str == '') {
                  $ncl_str = $ncl->id;
                } else {
                  $ncl_str .=",".$ncl->id;
            ...
            if(count($el->sub_accounts)>0) {

             $ncl_query = DB::SELECT(DB::raw("Select t.*, COALESCE(op.opening_amount,0) as op_amount, op.opening_amount_type FROM (Select sub_accounts.id as subAccId, sub_accounts.account_group_id, sub_accounts.sub_account_name, tt.* FROM sub_accounts LEFT JOIN (Select account_transitions.cash_bank_sub_account_id, (ABS(SUM(COALESCE(debit,0)) - SUM(COALESCE(credit,0)))) as amount, (CASE  WHEN SUM(ABS(COALESCE(debit,0))) > SUM(ABS(COALESCE(credit,0))) THEN 'debit' ELSE 'credit' END)  as amount_type FROM sub_accounts LEFT JOIN account_transitions ON sub_accounts.id = account_transitions.cash_bank_sub_account_id WHERE YEAR(transition_date) = ".$year. " AND cash_bank_sub_account_id IN (".$ncl_str.") AND is_cashbook=0 GROUP BY sub_accounts.id) as tt ON tt.cash_bank_sub_account_id = sub_accounts.id) as t LEFT JOIN (Select account_transitions.cash_bank_sub_account_id, (ABS(SUM(COALESCE(debit,0)) - SUM(COALESCE(credit,0)))) as opening_amount, (CASE  WHEN SUM(ABS(COALESCE(debit,0))) > SUM(ABS(COALESCE(credit,0))) THEN 'debit' ELSE 'credit' END)  as opening_amount_type From account_transitions LEFT JOIN sub_accounts ON sub_accounts.id = account_transitions.cash_bank_sub_account_id WHERE YEAR(transition_date) < ".$year." AND cash_bank_sub_account_id!=146 AND cash_bank_sub_account_id IN (".$ncl_str.") AND is_cashbook=0 GROUP BY cash_bank_sub_account_id) as op ON t.subAccId=op.cash_bank_sub_account_id WHERE t.amount IS NOT NULL OR op.opening_amount IS NOT NULL"));
              
            /*$ncl_query = DB::SELECT(DB::raw("Select t.*, COALESCE(op.opening_amount,0) as op_amount, op.opening_amount_type FROM (Select sub_accounts.account_group_id, account_transitions.cash_bank_sub_account_id, sub_account_name, (ABS(SUM(COALESCE(debit,0)) - SUM(COALESCE(credit,0)))) as amount, (CASE  WHEN SUM(ABS(COALESCE(debit,0))) > SUM(ABS(COALESCE(credit,0))) THEN 'debit' ELSE 'credit' END)  as amount_type FROM account_transitions LEFT JOIN sub_accounts ON sub_accounts.id = account_transitions.cash_bank_sub_account_id WHERE YEAR(transition_date) = ".$year. " AND cash_bank_sub_account_id IN (".$ncl_str.") AND is_cashbook=0 GROUP BY cash_bank_sub_account_id) as t LEFT JOIN (Select account_transitions.cash_bank_sub_account_id, (ABS(SUM(COALESCE(debit,0)) - SUM(COALESCE(credit,0)))) as opening_amount, (CASE  WHEN SUM(ABS(COALESCE(debit,0))) > SUM(ABS(COALESCE(credit,0))) THEN 'debit' ELSE 'credit' END)  as opening_amount_type From account_transitions LEFT JOIN sub_accounts ON sub_accounts.id = account_transitions.cash_bank_sub_account_id WHERE YEAR(transition_date) < ".$year." AND  cash_bank_sub_account_id IN (".$ncl_str.") AND is_cashbook=0 GROUP BY cash_bank_sub_account_id) as op ON t.cash_bank_sub_account_id=op.cash_bank_sub_account_id WHERE t.amount != 0"));*/ 
          } else {$ncl_query='';}


            $html.='<tr><td colspan="2"><b>Non Current Liability</b></td><tr>';

            if(!empty($ncl_query)) {
                
              foreach($ncl_query as $ncl_data) {   
                if($ncl_data->opening_amount_type == $ncl_data->amount_type) {
                  $amt = abs((float)$ncl_data->op_amount + (float)$ncl_data->amount);
                } else {
                  $amt = abs((float)$ncl_data->op_amount - (float)$ncl_data->amount);
                } 

                if($ncl_data->subAccId == 120) {
                  //add adjustment cost price in control account
                  $data_adj = DB::table("product_transitions")

                                ->select(DB::raw("COALESCE(SUM(CASE  WHEN transition_type = 'in' THEN cost_price ELSE 0 END),0) as adj_in, COALESCE(SUM(CASE  WHEN transition_type = 'out' THEN cost_price ELSE 0 END),0) as adj_out"))
                                ->whereYear('transition_date','<=',$request->year)
                                ->whereNotNull('transition_adjustment_id')->first();

                  $adj_in = 0; $adj_out =0;$adj_amount = 0;
                  if($data_adj) {
                      $adj_in = (int) $data_adj->adj_in;
                      $adj_out = (int) $data_adj->adj_out;
                  }

                  $adj_amount = $adj_in - $adj_out;

                 // $html.='<tr><td>'.$ncl_data->sub_account_name.'</td><td>'.($amt+$adj_amount).'</td></tr>';
                  $html.='<tr><td>'.$ncl_data->sub_account_name.'</td><td>'.$amt.'</td></tr>';
                } else {
                  $html.='<tr><td>'.$ncl_data->sub_account_name.'</td><td>'.$amt.'</td></tr>';
                }
            ...
            //$html.='<tr><td class="text-right"><b>Total</b></td><td>'.($total_ncl + $adj_amount).'</td><tr>';
            $html.='<tr><td class="text-right"><b>Total</b></td><td>'.($total_ncl).'</td><tr>';
          }
        //end non current liablity
        ...
        //start current liability
         ...
         if($el->name == 'Current Liability') {  
          ...
          $cl_query = DB::SELECT(DB::raw("Select t.*, COALESCE(op.opening_amount,0) as op_amount, op.opening_amount_type FROM (Select sub_accounts.id as subAccId, sub_accounts.account_group_id, sub_accounts.sub_account_name, tt.* FROM sub_accounts LEFT JOIN (Select account_transitions.sub_account_id, (ABS(SUM(COALESCE(debit,0)) - SUM(COALESCE(credit,0)))) as amount, (CASE  WHEN SUM(ABS(COALESCE(debit,0))) > SUM(ABS(COALESCE(credit,0))) THEN 'debit' ELSE 'credit' END)  as amount_type FROM sub_accounts LEFT JOIN account_transitions ON sub_accounts.id = account_transitions.sub_account_id WHERE YEAR(transition_date) = ".$year. " AND sub_account_id IN (".$cl_str.") AND is_cashbook=0 GROUP BY sub_accounts.id) as tt ON tt.sub_account_id = sub_accounts.id) as t LEFT JOIN (Select account_transitions.sub_account_id, (ABS(SUM(COALESCE(debit,0)) - SUM(COALESCE(credit,0)))) as opening_amount, (CASE  WHEN SUM(ABS(COALESCE(debit,0))) > SUM(ABS(COALESCE(credit,0))) THEN 'debit' ELSE 'credit' END)  as opening_amount_type From account_transitions LEFT JOIN sub_accounts ON sub_accounts.id = account_transitions.sub_account_id WHERE YEAR(transition_date) < ".$year." AND  sub_account_id IN (".$cl_str.") AND is_cashbook=0 GROUP BY sub_account_id) as op ON t.subAccId=op.sub_account_id WHERE t.amount IS NOT NULL OR op.opening_amount IS NOT NULL"));
    ...
    $total_eq_li = $total_eq + $total_cl + $total_lt + $total_ncl + $adj_amount;
