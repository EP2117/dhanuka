    ...
    public function getProductsForPurchaseInvoice($action, $id)
    {
        if ($action == "edit") {
            ...
            $data = DB::table("products")
                ...
                ->select(DB::raw("products.id as product_id, products.brand_id, products.category_id, CONCAT(products.product_name, ' - ', products.product_code , ' - ', categories.category_name) as product_name,products.selling_price,products.purchase_price,uom_id,uoms.uom_name,SUM(CASE  WHEN product_transitions.transition_type = 'in' THEN product_transitions.product_quantity  ELSE 0 END)  as in_count, SUM(CASE  WHEN product_transitions.transition_type = 'out' AND (product_transitions.transition_sale_id != " . $id . " OR product_transitions.transition_sale_id IS NULL) THEN product_transitions.product_quantity  ELSE 0 END)  as out_count,SUM(CASE  WHEN product_transitions.transition_type = 'out' THEN product_transitions.product_quantity  ELSE 0 END)  as total_out_count"))
            ...
            $data = DB::table("products")
                ...
                ->select(DB::raw("products.id as product_id, products.brand_id, products.category_id, CONCAT(products.product_name, ' - ', products.product_code, ' - ', brands.brand_name) as product_name,products.product_price,uom_id,uoms.uom_name,SUM(CASE  WHEN product_transitions.transition_type = 'in' THEN product_transitions.product_quantity  ELSE 0 END)  as in_count, SUM(CASE  WHEN product_transitions.transition_type = 'out' AND (product_transitions.transition_transfer_id != " . $id . " OR product_transitions.transition_transfer_id IS NULL) THEN product_transitions.product_quantity  ELSE 0 END)  as out_count, SUM(CASE  WHEN product_transitions.transition_type = 'out' THEN product_transitions.product_quantity  ELSE 0 END)  as total_out_count"))
            ...
            $data = DB::table("products")
                ...
                ->select(DB::raw("products.id as product_id, products.category_id, products.category_id, CONCAT(products.product_name, ' - ', products.product_code, ' - ', categories.category_name) as product_name,products.selling_price,products.purchase_price,uom_id,uoms.uom_name,SUM(CASE  WHEN product_transitions.transition_type = 'in' THEN product_transitions.product_quantity  ELSE 0 END)  as in_count, SUM(CASE  WHEN product_transitions.transition_type = 'out' THEN product_transitions.product_quantity  ELSE 0 END)  as out_count, SUM(CASE  WHEN product_transitions.transition_type = 'out' THEN product_transitions.product_quantity  ELSE 0 END)  as total_out_count"))
...
while($date < $end)  { 
   ...
   $range[] = date('Y-m-d', $date);
    ...
    public function getCOGSReport(Request $request)
    {
        $month = Carbon::now()
        ->year($request->year)
        ->month($request->month);
        ...
        $data = DB::table("product_transitions")
            ...
            ->select(DB::raw("product_id, products.product_name,products.product_code,product_transitions.transition_date, (CASE  WHEN transition_sale_id IS NOT NULL AND transition_return_id IS NULL THEN ROUND(product_transitions.cost_price)  ELSE 0 END)  as  cost_price, (CASE  WHEN transition_return_id IS NOT NULL THEN ROUND(product_transitions.cost_price)  ELSE 0 END)  as  cn_cost_price, product_transitions.product_quantity, CASE  WHEN transition_sale_id IS NOT NULL THEN CONCAT('Sale: ', sales.invoice_no) WHEN transition_return_id IS NOT NULL THEN CONCAT('Sale Return: ', sale_returns.return_no) WHEN transition_debit_note_id IS NOT NULL THEN CONCAT('Debit Note: ', debit_notes.credit_note_no) WHEN transition_transfer_id IS NOT NULL AND transition_type='out' THEN CONCAT('Transfer: ', transfers.transfer_no) WHEN transition_transfer_id IS NOT NULL AND transition_type='in' THEN CONCAT('Receive: ', transfers.transfer_no) WHEN transition_adjustment_id IS NOT NULL AND transition_type='out' THEN CONCAT('Adjustment Out: ', inventory_adjustment.invoice_no) WHEN transition_adjustment_id IS NOT NULL AND transition_type='in' THEN CONCAT('Adjustment In: ', inventory_adjustment.invoice_no)
                WHEN transition_entry_id IS NOT NULL THEN CONCAT('Main Warehouse Entry: ', mainwarehouse_entries.entry_no) ELSE '' END as transition_no"))
            ...
            ->leftjoin('debit_notes', 'debit_notes.id', '=', 'product_transitions.transition_debit_note_id')
        ...
        if($request->type_id == 'sale') {
            $data->whereNotNull('transition_sale_id');
            $data->whereNull('transition_return_id');
        } else if($request->type_id == 'credit_note'){
            $data->whereNotNull('transition_return_id');
        } else {
            $data->where(function($query) {
                    $query->whereNotNull('transition_sale_id')
                          ->orwhereNotNull('transition_return_id');
                    /*$query->whereNotNull('transition_sale_id')
                          ->orwhereNotNull('transition_entry_id')
                          ->orwhereNotNull('transition_credit_note_id')
                          ->orwhereNotNull('transition_debit_note_id')
                          ->orwhereNotNull('transition_adjustment_id')
                          ->orwhereNotNull('transition_transfer_id');*/
                });
        }
        ...
        if($request->month != ""){
            $data->whereMonth('product_transitions.transition_date','=',$request->month);
        }
        $request->year=$request->year==null ? Carbon::now()->year :  $request->year;
        if($request->year != ""){
            $data->whereYear('product_transitions.transition_date','=',$request->year);
        }
        ...
        if ($request->product_name != "") {
            $data->where('products.id', $request->product_name);
        }
        $data  = $data->orderBy("product_id")->paginate($limit);

        //total query
        $total_data = DB::table("product_transitions")

                        ->select(DB::raw("(SUM(CASE  WHEN product_transitions.transition_sale_id IS NOT NULL && product_transitions.cost_price IS NOT NULL AND transition_return_id IS NULL THEN FLOOR(product_transitions.cost_price) ELSE 0 END) - SUM(CASE  WHEN product_transitions.transition_return_id IS NOT NULL && product_transitions.cost_price IS NOT NULL THEN FLOOR(product_transitions.cost_price) ELSE 0 END)) as total_cost_price"))

                        ->leftjoin('products', 'products.id', '=', 'product_transitions.product_id');

        if($request->type_id == 'sale') {
            $total_data->whereNotNull('transition_sale_id');
            $total_data->whereNull('transition_return_id');
        } else if($request->type_id == 'credit_note'){
            $total_data->whereNotNull('transition_return_id');
        } else {
            $total_data->where(function($query) {
                    $query->whereNotNull('transition_sale_id')
                          ->orwhereNotNull('transition_return_id');
                    /*$query->whereNotNull('transition_sale_id')
                          ->orwhereNotNull('transition_entry_id')
        ...
        if ($request->from_date != '' && $request->to_date != '') {
            $total_data->whereBetween('product_transitions.transition_date', array($request->from_date, $request->to_date));
        } else if ($request->from_date != '') {
            $total_data->whereDate('product_transitions.transition_date', '>=', $request->from_date);
        } else if ($request->to_date != '') {
            $total_data->whereDate('product_transitions.transition_date', '<=', $request->to_date);
        } else {
        }

        if($request->month != ""){
            $total_data->whereMonth('product_transitions.transition_date','=',$request->month);
        }
        $request->year=$request->year==null ? Carbon::now()->year :  $request->year;
        if($request->year){
            $total_data->whereYear('product_transitions.transition_date','=',$request->year);
        }

        if ($request->product_code != "") {
            $total_data->where('products.product_code', 'LIKE', "%$request->product_code%");
        }
        if ($request->product_name != "") {
            $total_data->where('products.id', $request->product_name);
        }

        $total_data = $total_data->value('total_cost_price');
        $total_cost_price = $total_data;
        //end total query
        return response(compact('data','total_cost_price'), 200);
    }
}
