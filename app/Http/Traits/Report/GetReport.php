...
trait GetReport
{
    ...
    public function getPurchaseOutStandingReport($request)
    {
        ...
        //$sup = PurchaseInvoice::where('payment_type', 'credit');
        $sup =     DB::table("purchase_invoices")
                        ->select(DB::raw("purchase_invoices.*,suppliers.name as supplier_name,suppliers.supplier_code"))
        ...
        if ($request->state_id) {
            ...
            $sup->where('suppliers.state_id', $request->state_id);
        }
        if ($request->township_id) {
           /* $town = $request->township_id;
            $sup->whereHas('supplier', function ($q) use ($town) {
            ...
            $sup->where('suppliers.township_id', $request->township_id);
        }
        if ($request->from_date != '' && $request->to_date != '') {
            $sup->whereBetween('invoice_date', array($request->from_date, $request->to_date));
        } else if ($request->from_date != '') {
               ...
               // $invoices = PurchaseInvoice::with('payments')->where('supplier_id', $s->supplier_id)->where('payment_type', 'credit');
                ...
                foreach ($invoices as $k => $i) {
                    ...
                    $payments = DB::table("collection_purchase")
                                    ...
                                    ->select(DB::raw("collection_purchase.*"))
                                    ->where('collection_purchase.purchase_id',$i->id)->get();
                    $gain_loss = 0;
                    $gain_amt = 0;
                    $loss_amt = 0;
                    if (!empty($i->payments)) {
                        ...
                        $loss = 0;
                        foreach ($i->payments as $pay) {
                            $gain += $pay->gain_amount;
                            $gain_amt += $pay->gain_amount;
                            $loss += $pay->loss_amount;
                            $loss_amt += abs($pay->loss_amount);
                        }
                        $gain_loss = $gain + $loss;
                    }
    ...
    public function getSaleOutstandingReport($request)
    {
        ...
        if (isset($request->currency_id) && $request->currency_id != 1) {
            //$cus = Sale::where('payment_type', 'credit')->whereRaw('(total_amount_fx-(IFNULL(cash_discount_fx,0) + pay_amount_fx + collection_amount_fx)) > 0');
            $cus = DB::table("sales")
                         ->select(DB::raw("sales.*,ps.products,townships.township_name,states.state_name"))
                        ->where('payment_type', 'credit')
                        ->whereRaw('(total_amount_fx-(IFNULL(cash_discount_fx,0) + pay_amount_fx + collection_amount_fx)) > 0');
        } else {
            ...
            $cus = DB::table("sales")
                        ->where('payment_type', 'credit')
                        ->whereRaw('(total_amount-(IFNULL(cash_discount,0) + pay_amount + collection_amount + return_amount + customer_return_amount)) > 0');
        }
        ...
        $cus->leftjoin(DB::raw("(SELECT product_sale.sale_id, GROUP_CONCAT(products.product_name) as products
                            FROM product_sale LEFT JOIN products ON products.id = product_sale.product_id 
                            GROUP BY product_sale.sale_id

                            ) as ps"), function ($join) {

                $join->on("ps.sale_id", "=", "sales.id");
            });
        ...
        if ($request->product_name != "") {
            $cus->where('ps.products', 'LIKE', "%$request->product_name%");
        ...
        if ($request->state_id) {
            ...
            $cus->where('customers.state_id',$request->state_id);
        }
        if ($request->township_id) {
            /*$town = $request->township_id;
            ...
            $cus->where('customers.township_id',$request->township_id);
        }
        if ($request->from_date != '' && $request->to_date != '') {
        ...
        if ($cus->isNotEmpty()) {
            foreach ($cus as $key => $s) {
                ...
                $per_gain_loss_amt = $per_inv_amt = $per_paid_amt = $per_bal_amt = 0;
                $p_outstandings[$key] = new \stdClass();
               // $invoices = Sale::with('collections','customer', 'customer.township')->where('customer_id', $s->customer_id)->where('payment_type', 'credit');

                $invoices = DB::table("sales")
                              ->select(DB::raw("sales.*,sale_men.sale_man,customers.cus_code,customers.cus_name,customers.cus_phone,ps.products,townships.township_name,states.state_name"))
                              ->where('customer_id', $s->customer_id)
                              ->where('payment_type', 'credit');

                $invoices->leftjoin('customers', 'customers.id', '=', 'sales.customer_id');
                $invoices->leftjoin('sale_men', 'sale_men.id', '=', 'sales.office_sale_man_id');
                $invoices->leftjoin('states', 'states.id', '=', 'customers.state_id');
                $invoices->leftjoin('townships', 'townships.id', '=', 'customers.township_id');
                $invoices->leftjoin(DB::raw("(SELECT product_sale.sale_id, GROUP_CONCAT(products.product_name) as products
                            FROM product_sale LEFT JOIN products ON products.id = product_sale.product_id 
                            GROUP BY product_sale.sale_id
                ...
                if ($request->product_name != "") {
                    $invoices->where('ps.products', 'LIKE', "%$request->product_name%");
                ...
                foreach ($invoices as $k => $i) {
                    ...
                    $products = DB::table("product_sale")
                                ...
                                ->select(DB::raw("product_sale.*,products.product_name"))
                                ->leftjoin('products', 'products.id', '=', 'product_sale.product_id')
                                ->where('product_sale.sale_id',$i->id)->get();

                if($products) {
                    $invoices[$k]->products = $products;
                }
                    ...
                    $collections = DB::table("collection_sale")
                                ...
                                ->select(DB::raw("collection_sale.*"))
                                ->where('collection_sale.sale_id',$i->id)->get();

                    if (!empty($collections)) {
                        // dd($i->payments);
                        $gain = 0;
                        $loss = 0;
                        foreach ($collections as $pay) {
                            $gain += $pay->gain_amount;
                            $gain_amt += $pay->gain_amount;
                            $loss += $pay->loss_amount;
                            $loss_amt += abs($pay->loss_amount);
                ...
                $p_outstandings[$key]->state_name = $s->state_name;
                $p_outstandings[$key]->township_id = $s->township_id;
                $p_outstandings[$key]->township_name = $s->township_name;
                $p_outstandings[$key]->out_list = $invoices;
                $p_outstandings[$key]->total_inv_amt = $per_inv_amt;
                $p_outstandings[$key]->total_paid_amt = $per_paid_amt;
    ...
    public function getSaleOutstandingReport_OLD($request)
    {
        ini_set('memory_limit', '2048M');
        ini_set('max_execution_time', 360);
    ...
    public function getValuation_old($request)
    {
        $where = "";
        $p_where = "";
        $s_where = "";
    ...
    public function getValuation($request)
    {
        ...
        $products = DB::table("products")
            ->where('products.is_active', 1)
            ->select(DB::raw("products.id as product_id,products.selling_price,pt.transition_date,pt.cost_price,pt.in_cost_price,products.purchase_price,pt.entry_qty,products.product_name,products.minimum_qty, products.brand_id,pt.warehouse_id, products.product_code,uom_id,uoms.uom_name,brands.brand_name,categories.category_name, pt.in_qty,pt.out_qty,products.cost_price as product_cost_price,pt.adj_in_cost_price,pt.adj_out_cost_price"))
            ->leftjoin(DB::raw("(SELECT product_id,product_quantity,transition_type,transition_purchase_id, warehouse_id, transition_date, branch_id,
                         SUM(CASE  WHEN transition_type = 'in'  AND transition_entry_id IS NOT NULL THEN product_quantity  ELSE 0 END) as entry_qty,
                         SUM(CASE  WHEN transition_type = 'in' AND transition_entry_id IS NULL THEN product_quantity  ELSE 0 END) as in_qty,
                         SUM(CASE  WHEN transition_type = 'out' AND transition_adjustment_id IS NOT NULL THEN cost_price  ELSE 0 END) as adj_out_cost_price,
        ...
        //ps.s_valuation_amount,ps.s_qty,pp.p_valuation_amount,
        //dd($data);
        return $data;
    }
