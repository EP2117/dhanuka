        ...
        $(".txt_product").on("select2:select", function(e) {
          ...
          if(app.isMMK) {
           $("#unit_price_"+row_id).val(purchase_price);
            ...
            /*if(app.isEdit) {
              var p_available_qty = parseFloat(app.products[key].in_count) - parseFloat(app.products[key].total_out_count);  

              if(parseInt(p_available_qty) <= 0) {
                $("#qty_"+row_id).attr('readonly', 'readonly');
                $("#unit_price_"+row_id).attr('readonly', 'readonly');
              }
            }*/
            ...
            axios.get("/purchase/" + id+'/edit')
                .then(function(response) {
                    ...
                    $.each(app.ex_products, function( key, product ) {
                        ...
                        var pkey = app.products.findIndex(x => x.product_id == product.id);
                        var p_available_qty = parseFloat(app.products[pkey].in_count) - parseFloat(app.products[pkey].total_out_count);  
